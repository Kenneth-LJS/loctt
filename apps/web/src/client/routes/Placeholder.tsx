  ...
  return (
    <main className="min-h-screen bg-bg-canvas text-text-primary p-8">
      <div className="max-w-3xl mx-auto flex flex-col gap-6">
        ...
        <nav className="flex flex-wrap gap-1 p-1 rounded-(--radius-md) bg-bg-muted border border-border-subtle text-xs">
          {NAV.map(n => (
            <Link
              key={n.to}
              to={n.to}
              className="px-2.5 py-1 rounded-(--radius-sm) text-text-secondary hover:text-text-primary hover:bg-bg-surface"
              activeProps={{ className: "px-2.5 py-1 rounded-(--radius-sm) bg-bg-surface text-text-primary shadow-(--shadow-raised)" }}
            >
              {n.label}
            </Link>
          ))}
