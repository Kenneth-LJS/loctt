...
const PATHS: Record<IconName, ReactElement> = {
  ...
  // UI-3: the *vertical* kebab. `more` (horizontal) means "this whole
  // surface"; this one means "this one item" — a board column, a row.
  // Two glyphs so the two scopes are not the same button drawn twice.
  ...
  // dot from lucide icons"). Filled, so it reads at 12px.
  ...
  // K107. Underline: the conventional "U over a rule" — a stem that
  // curves into a bowl, with the baseline rule beneath it. Highlight: a
  // marker nib angled over a thick stroke, which is how every editor
  // draws it; the nib is what distinguishes it from `underline`.
  underline: <><path d="M4.5 3v4.5a3.5 3.5 0 007 0V3" /><path d="M3.5 13.5h9" /></>,
  highlight: <><path d="M9.5 2.5l4 4-5 5H5l-1.5-1.5z" /><path d="M3.5 13.5h9" /></>,
  ...
  // UI-23b: was two open hooks (`M4 4.5h3.5v3.5A3 3 0 014 11` ×2) which
  // drew as `ᴊᴊ` — a pair of descending J shapes, not quotation marks.
  // Now each mark is a filled-looking comma pair: a bowl (the round part)
  // with a tail, mirrored for the second mark, which is how a quote glyph
  // actually reads at 16px. Name and wiring were already correct.
...
const FILLED = new Set<IconName>(["more", "moreVertical", "star", "dot", "drag", "bold"]);
