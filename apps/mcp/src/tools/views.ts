...
const FilterSchema = z.discriminatedUnion("kind", [
  ...
  z.object({
    ...
    query: z.string().min(1).describe(
      ...
      + "Validated on write and spacing-normalized. ONLY spacing changes; nothing "
      + "else is rewritten.",
    ),
  }),
]);
...
const FILTERS_GUIDANCE =
  ...
  + "supplied. They are never merged into a single DSL string and never reordered. Each entry is "
...
export const TOOLS: readonly ToolDef[] = [
  ...
  },
  {
    ...
    description:
      ...
      "It is a property of the view, never a filter term. `icon` is an optional display icon, and " +
  ...
  },
  {
    ...
    description:
      ...
      "rejected, so use the id). Any of `name`, `filters`, `sort`, `archivedScope`, `icon` " +
  ...
  },
  {
    ...
    description:
      ...
      "deleted here, but only with `replaceBroken: true` as well, because deleting it discards " +
