...
- [ ] **Read-only views + degraded entries (ERR-10, LST-51)** — RULED A-PRESCAN-2: alert at point-of-use (the view), not only Settings. **TML-48 is now resolved separately** (field-local degrade → flagged in the timeline lane; A-TML48-RESOLVE). Remaining: `/list` must surface a `role="alert"` off the workflow config's `broken` entries (the way SET-33/K32's `WorkflowPanelFrame` does in settings), then un-fixme the ERR-10/LST-51 specs. Build only.
