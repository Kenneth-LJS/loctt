...
- [ ] **Visual nested query builder + JQL-like DSL (LST-40/44/45; K80/K83)** — **partially done; the visual builder UI + remaining K80 date functions remain.** Progress: ✅ K77 `is empty`/`is not empty` + `is null` (done); ✅ `currentUser()` (A-K80-CURRENTUSER); ✅ `IN (…)`/`not in`/`~` (already existed); ✅ **the "second label widens results" bug** — fixed via a per-field All/Any toggle on the label filter (A-LABELS-MATCH: `labels_match=all` → `labels = a and labels = b`, default OR unchanged). ✅ **K80 date functions** — DONE (A-K80-DATES): `now()` + `startOf/endOf Day/Week/Month` with signed offsets, across parser/evaluator/validate + CLI/MCP/web threading + docs; `@verifies K80` at every layer, red-proven. **Still open:** the **visual nested builder UI** over the DSL only (`validateQuery` is its missing caller; K83 design — refuse-on-unrenderable, coexist as "Advanced", serialize to `q`, NOT deferred to v2). LST-44/45 (query error surfacing) are M1 blockers, already built.
