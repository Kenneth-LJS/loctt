...
- [~] **The git-sync engine (GIT-8,9,16,19,21,22,23,25,29,30,33,34,35,36)** — **STALE LINE CORRECTED (A188).** The reconcile engine already exists, is wired on all three surfaces, and is tested (A121/commit e99bfec) — the "no engine exists" framing was written against the superseded A68 ruling. What actually remains is a residual set of scoped features, split by risk:
  ...
  - [ ] **GIT-23** — 500-task sync progress + honest counts. SAFE, in progress.
  - [ ] **Safe remainder (implementer-decidable):** GIT-22 fstype enable-warning, GIT-34/36 named errors, GIT-19 open-tab UI, GIT-16 delete-vs-edit row.
  ...
  - [ ] **RULED (2026-09-16) — now buildable:** GIT-8/9/33 rekey preview+confirm per **K92** (UI confirms via preview; CLI/MCP auto-apply+report); GIT-35 newer-schema per **K94** (read remote `.schema-version`, refuse if newer, write nothing). Ties to H2 in release-readiness.md.
- [~] **git-sync UI cases** — the git UI already exists (`GitSyncPanel.tsx`, `ReconcilePanel.tsx`), so ~24 already-built GIT-* cases are liftable into `flow-git-sync.md` now; the UI half of the HELD/in-progress cases lands with each engine step above.
...
- [ ] **REL-16 — attachment PNG thumbnail** — LARGER than a web tweak: a bare inline `<img src>` reintroduces the exact script-execution risk REL-38's `octet-stream + nosniff` download endpoint defends against. Per **K14 (Ken's ruling)** the real shape is validate-on-upload + crop/compress pipeline (generalise `copyAvatar`'s sharp pipeline) + a cropper UI + a safe image endpoint, AND it **conflicts with REL-18's byte-identical-storage guarantee** for images (flagged unresolved in K14). Needs its own ticket + a REL-18 reconciliation (possibly a Ken call). Not a quick fix — deferred as a proper feature build.
