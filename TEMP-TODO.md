...
- [x] **A11Y-40 — `--text-tertiary`** — **STALE (text) + harness added.** Verified `--text-tertiary` already clears AA (5.81:1); the 3.67:1 was the old #7B8699. Static contrast harness now guards all text tokens ≥4.5:1 both themes (`styles/contrast.test.ts`). Full A11Y-40 in-situ axe scan still pending (see A11y §).
