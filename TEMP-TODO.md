...
- [ ] **Deep-linking (NEW workstream, K76)** — IN PROGRESS. ✅ Audit + plan written (`docs/dev/deep-linking-audit.md`): the target inventory (Tier 1 settings scroll-to-field + task comments/comment + copy-link; Tier 2 other task sections; Tier 3 already-covered list state), the hash-based scheme, and the 5-step build sequence. ✅ **Step 1: `useScrollToHash`** — the shared scroll-into-view + brief-highlight hook (hash-driven, retries for late-mounting targets, bounded so a deleted target doesn't loop; reduced-motion drops the flash), wired at `AppShell`, unit-tested + red-proven. **Remaining:** step 2 comments anchor + scroll-to-comment; step 3 copy-link-to-comment; step 4 settings scroll-to-field (unblocks K75 GUI nudge); step 5 Tier-2 task sections.
