...
- [ ] **Visual nested query builder + JQL-like DSL (LST-40/44/45; K80/K83)** — **partially done; the visual builder UI + remaining K80 date functions remain.** Progress: ✅ K77 `is empty`/`is not empty` + `is null` (done); ✅ `currentUser()` (A-K80-CURRENTUSER); ✅ `IN (…)`/`not in`/`~` (already existed); ✅ **the "second label widens results" bug** — fixed via a per-field All/Any toggle on the label filter (A-LABELS-MATCH: `labels_match=all` → `labels = a and labels = b`, default OR unchanged). **Still open:** (1) the **visual nested builder UI** over the DSL (`validateQuery` is its missing caller; K83 design — refuse-on-unrenderable, coexist as "Advanced", serialize to `q`); (2) K80 **date functions** (`now`, `startOf/endOf Day/Week/Month` + offsets — needs a stubbable clock; reconcile `~`/contains with header search). LST-44/45 (query error surfacing) are M1 blockers, already built.
...
- [ ] **Config pickers break past 1000 entries** — **NEARLY DONE (K90, ties NEW-25).** Rulings in K90; agent-level notes in A-K90-SEARCH. ✅ **Backend (committed `0f82bde`):** core `filterByName`/`filterProjects` + `?q=` on all five web endpoints + CLI `--filter`/pagination + MCP `q`/pagination + truncation footer + reference docs; core 12 / server 6 / integration 9 tests, all red-proven. ✅ **Labels picker (the load-bearing correctness fix, uncommitted):** `LabelsField` queries `searchLabels` (`?q=`, debounced) for candidates AND the server-authoritative create-offer gate, so a label past the 1000 window is found and never offered as a duplicate; `@verifies K90` at 1100 labels, red-proven; wired to create modal + task detail. **Remaining sub-step ONLY:** give `OptionPicker` an opt-in querying mode so the milestone/sprint/user/project pickers also search server-side (K90's "the dropdown itself searches" — those four have no inline-create hazard, so it's a find-past-1000 gap, not corruption). Then supersede A43 in decisions + drop the known-gaps entry.
