...
- [ ] **Config pickers break past 1000 entries** — **IN PROGRESS (K90, ties NEW-25).** Rulings locked in K90: name search is a core capability on all three surfaces (not web-only); UI pickers query (`?q=`), CLI/MCP paginate ≤1000/page; projects match name+slug+prefix; the duplicate-create guard must be server-authoritative. ✅ **Step 1 DONE (uncommitted):** core `config/search.ts` (`filterByName`/`filterProjects`/`isBlankQuery`, unit-tested + red-proven) + `?q=` wired into all five web list handlers (labels/milestones/sprints/users/projects), filter applied before pagination+counts so `total` reflects matches; `server.config-search.test.ts` (6 tests, red-proven). **Remaining:** (2) CLI/MCP — `?q=`/filter + pagination on the list commands/tools + reference docs; (3) UI — querying pickers (LabelsField + OptionPicker search mode) replacing fetch-and-filter; (4) the server-authoritative exact-name existence check so the create modal never offers a duplicate of a label outside the fetched window (the load-bearing correctness fix); (5) supersede A43 in decisions, drop the known-gaps entry.
