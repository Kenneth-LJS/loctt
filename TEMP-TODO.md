...
- [~] **K71 — dialog focus-trap / inert / restore** — **IN PROGRESS.** Added `ui/ConfirmDialog.tsx` (`ConfirmDialog`+`TypedConfirmDialog` over Modal). Migrated 5 of 7 §A1 dialogs (DeleteView, DeleteComment, DeleteConfirm, MoveTask, BodyConflict); real-browser focus-trap+restore e2e green + behavior specs preserved. **Remaining:** `AdvancedQueryEditor` + `CreateTaskModal`/DiscardDialog. See design-review §A1, A-K71.
