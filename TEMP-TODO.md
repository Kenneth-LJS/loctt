...
- [~] **The git-sync engine (GIT-8,9,16,19,21,22,23,25,29,30,33,34,35,36)** — **STALE LINE CORRECTED (A188).** The reconcile engine already exists, is wired on all three surfaces, and is tested (A121/commit e99bfec) — the "no engine exists" framing was written against the superseded A68 ruling. What actually remains is a residual set of scoped features, split by risk:
  ...
  - [ ] **HELD FOR KEN (extremely-devastating — do NOT build autonomously, see A188 § escalation):** GIT-21 force-push detection (live data-loss hazard; touches `last_synced_commit`), GIT-8/9/33 rekey preview+confirm (reissues a key, P-7), GIT-35 newer-schema refusal (migration-adjacent). Ties to H2 in release-readiness.md.
