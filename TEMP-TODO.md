...
- [~] **DS-BORDER-SUBTLE below 3:1 (K82) — NEEDS RE-RULING.** Building the contrast harness proved K82's stated fix insufficient: `--border-default` is only ~1.4:1 (light) / ~1.36:1 (dark), nowhere near 3:1. A divider that truly clears 3:1 must be ~`#7C8598`-dark — a prominent gridline, a real visual change. Structural dividers (row seps, footers, header cells) were migrated subtle→default (a visible improvement, harmless), but 3:1 is NOT met. **Decision needed:** (a) accept row separators as decorative (WCAG 3:1 for non-text applies to indicators *required* to understand content — arguably table rows aren't, if layout is clear) and drop the 3:1 target for dividers; or (b) introduce a genuine ≥3:1 `--border-divider` (dark gridlines everywhere). Harness marks these `.todo` pending the call.
