...
| 5 · UI build | 🔵 | **M1 through its gate** (round 8: 0 blockers, 0 majors). **M2.1–M2.5a committed**; M2.5b (attachments) is the last M2 subsection. Coverage **442/937**, UI suite 321. Four core defects surfaced by probing before building — an unreadable `task.md` reporting as "task not found", an orphaned enum freezing every write to a task, plus `bodyToken` and `unarchiveView` built in core with no callers. |
...
| M2.5b | attachments | *(uncommitted)* | 7 of 7: REL-35..41 | The API was already right on four of the seven (probe), so this is the client: `AttachmentsPanel`, a `postFile` transport for multipart, and two hooks. **REL-35's message is a client fix, decided not stopped** — the server says `upload exceeds maximum size of 52428800 bytes`, and the browser knows the name and the byte count before it sends, so the refusal names all three in the user's units and nothing leaves. The server check is untouched. **REL-40 is not a batch endpoint** — the case asks for per-file outcome, never for one request, so it is twenty sequential POSTs and `multipart.ts` is unchanged. **Two mutations rejected for going red on a timeout rather than an assertion**: REL-40's first attempt left rows stuck `pending` and failed inside the shared `settled` helper, so the helper was made non-asserting; the removal test's first attempt failed on a `waitForResponse` for a URL the mutation had changed. **A mutation that did not mutate**: neutralising `basename` in `multipart.ts` left REL-36 green — core's `attachFile` is the only load-bearing traversal guard, and the web layer's copy is redundant (`known-gaps.md`). **An aliasing gap found, not fixed**: on macOS `DELETE .../attachments/DROP.TXT` removes `drop.txt` and writes history naming a file that never existed; the behaviour differs on a case-sensitive volume (`known-gaps.md`). Unit 2547 unchanged, UI 321 → 329. |
