...
A task with direct children on the tree axis (the `parent`/`child` relationship, or whatever `workflow.yaml` renames it to) carries a **`children`** roll-up: `{done, active, total, discarded}`. The counts are by status *category*, not by a specific status key — `done` is children in a `completed`-category status, `active` those in an `active`-category status, and `total` is every child except those in a `discarded`-category status (`discarded` reports how many were excluded). This is the same exclusion milestone and sprint progress use, so an abandoned child does not hold the total below completion forever. The un-started remainder is `total - done - active`. The `children` key is omitted when the task has no children or the tracker has no tree axis, so its presence always means something.
...
When a task was **deleted on one side and edited on the other** since the last sync, that is surfaced too — as a whole-task keep-deletion / keep-task decision, not a per-field conflict. The reconcile output names the task by key and states which side deleted it and which edited it. It is not resolved by either side winning silently. Resolution is web-UI-primary (Settings → Sync); keeping a task whose key would then collide is routed through the normal rekey summary, never resurrected with a colliding key silently.

When a merge leaves two tasks in the same project sharing a key (two clones each created a task offline that landed on the same key), one is renumbered — the earlier `created_at` keeps the key, the ULID `id` breaks a tie, and the renumbered task's old key is kept in `key_history` so it still resolves. `sync_from_git` applies this automatically (MCP is request/response and stays scriptable) and names each renumber, old key → new key (`Renumbered N task(s) to resolve key collisions:` followed by `  T-2 → T-3` lines). The web UI instead shows a preview and waits for a confirm before renumbering. A collision that cannot be renumbered — a task whose project has no key counter yet — is reported as an unresolved key (`Warning: N key collision(s) remain unresolved: …. Run 'loctt doctor'.`) rather than silently leaving two tasks sharing a key.

If the `loctt` branch was force-pushed or its history rewritten so that the last-synced commit is no longer part of it, `publish_to_git` / `sync_from_git` **refuse and write nothing** — this is reported distinctly from a conflict. The output names the commit that can no longer be found and the remote, and states that local files and `last_synced_commit` are unchanged. LocTT does not silently re-base onto the new head (that would discard local changes made since the missing commit); recovery is the user's, done in git. Tell the user to inspect the rewritten branch (`git log loctt`) against their local `.loctt/`, and — once they have reviewed and merged the two by hand — to re-establish a base explicitly in git (e.g. `git branch -f loctt <commit>`) and sync again. Do not attempt an automated rebase or base-reset on their behalf.

If the `loctt` branch was written by a **newer version of LocTT** than the one running (its `.schema-version` is higher than this installation understands), `publish_to_git` / `sync_from_git` **refuse and write nothing** — reported distinctly from a conflict or a rewrite. The output names both schema versions (the branch's and this build's). Schema travels through `loctt migrate`, never through sync, so LocTT will not apply a branch it cannot read (that could corrupt or drop data). The fix is to **upgrade LocTT**, not to migrate — the branch is already ahead of what this build can read. Tell the user to upgrade their LocTT installation, then sync again; do not suggest `loctt migrate`. A branch with no `.schema-version`, or one at the same/older version, syncs normally; a present-but-unreadable version is also refused (it cannot be proven safe to read).

If the `loctt` branch carries a task whose `task.md` does **not parse** (malformed frontmatter), `sync_from_git` does not abort — it applies the rest of the sync (the counts still report) and then names each unparseable task by id and path (`Warning: N synced task(s) could not be parsed (the rest of the sync was applied). Inspect:` followed by `  <id>: <path> — <reason>` lines). The bad file is kept exactly as it came from the branch, not rewritten and not silently dropped; it shows up as a broken-file task in listings and in `loctt doctor`. Tell the user which named `task.md` to fix — do not treat the whole sync as failed.

If LocTT's temporary publish/sync worktree (under `.loctt/local/`) is missing but git still has it registered — the directory was deleted by hand while git had it locked — `publish_to_git` / `sync_from_git` **refuse and write nothing** (local task files are not modified) rather than surface git's opaque `missing but locked worktree` error. The output names the specific worktree and gives two repair paths, each stating it leaves local task files as they are: re-establish the worktree in git (`git worktree prune`, or `git worktree remove --force <path>` / `git worktree unlock <path>` if locked), or `loctt git disable` then `loctt git enable`. Relay the named repair path to the user; do not retry blindly (the same broken worktree fails again).

### `get_reconcile_status`

Returns structured JSON for an in-progress reconciliation, or `{ "in_progress": false }` when none. When in progress, reports `mode`, `base_commit`, `remote_commit`, `started_at`, and a `conflicts` array — each with `task_key`, `field`, `field_label`, `kind`, `local`, `remote`, and `remote_drift` / `local_drift` (the reason a value references config or a task missing locally, else null) — a `delete_vs_edit` array (tasks deleted on one side and edited on the other; each with `task_key`, `task_title`, `deleted_side`, `edited_side`) — plus `auto_merged` (fields that merged or converged without a conflict). No parameters. Use it to explain to the user what must be resolved before publish/sync can complete.
