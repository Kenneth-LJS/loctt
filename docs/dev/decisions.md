...
### A191 · GIT-21 force-push / history-rewrite — detect (against the TRUE remote head) and refuse, per K93

**Ticket:** GIT-21 (K93 is the binding ruling) · **Date:** 2026-09-16 · **Commit:** uncommitted (working tree).

**Situation.** A remote force-push that no longer contains `last_synced_commit` was treated as ordinary divergence: `planSync` saw "no base," reclassified everything, and could take incoming over local edits whose base is gone (A188 H-e, live data loss). K93 ruled: detect via `git merge-base --is-ancestor`, refuse with a distinct error, write NOTHING, offer no automated recovery (no rebase / base-reset / `last_synced_commit` mutation) — recovery is the user's, in git.

**What was built.**
- **Core (`git/publish-sync.ts`):** `GitHistoryRewrittenError extends GitSyncError` (carries `missingCommit`/`remoteHead`/`branch`/`remote`); `assertNotHistoryRewrite()` runs `git merge-base --is-ancestor <base> <head>` and refuses on exit 1 (not an ancestor) OR exit 128 (base unresolvable — rewritten AND pruned), proceeding only on exit 0; a null status (git could not spawn) is left to existing broken-git handling. Wired into **both** divergence paths before `planSync`: `pullFromLocttBranch` (sync) and `detectPublishReconcile` (publish). Exported from `git/index.ts` + top-level `index.ts`.
- **The non-obvious correctness point (`resolveRemoteHeadForAncestry`).** A force-push is a non-fast-forward fetch, which git REFUSES to apply to the local `loctt` branch ref — so after a rewrite the local `git rev-parse loctt` still returns the stale base, while the remote-tracking ref `refs/remotes/<remote>/<branch>` force-updates to the true head. Reading the local ref alone would miss exactly the rewrite this guard exists to catch. So the ancestry check resolves the head from the tracking ref (verified via `rev-parse --verify --quiet`, because a bare `rev-parse <missing-ref>` prints the ref name to stdout and exits non-zero — a trap the existing publish-sync tests caught), falling back to the local ref when there is no remote. The guard also runs BEFORE the `remoteHead === base` no-op / fast-forward short-circuits, so a stale local ref does not swallow it.
- **Server (`apps/web/src/server/server.ts`):** `gitErrorResponse` maps it to **409**, `code: "history_rewritten"`, `data_state: "not_saved"`, `recovery: { kind: "none" }`, plus a `history_rewritten` payload; `error()` allowlist extended to pass that payload through (it drops unknown envelope keys). New `ErrorCode` `"history_rewritten"` + optional `ErrorResponse.history_rewritten` in contracts.
- **Web panel (`GitSyncPanel.tsx`):** `historyRewritten()` detector + `HistoryRewrittenRefusal` banner — names the rewritten branch/remote + missing commit, states nothing was written, gives the two user-does-**in-git** next actions (inspect / re-establish base), and offers NO action controls (asserted: 0 buttons). Also **hardened `reconcileBlocked` to branch on the error CODE, not `/reconcil/i` on the message** — the refusal legitimately mentions "merged … by hand," and the old substring match mislabelled it as a reconcile and hid the banner. No green test asserted the old substring behaviour (the reconcile e2e drives a real sentinel), so this is a robustness fix, not a bug-encoding test edit.
- **CLI (`apps/cli/commands/git.ts`) + MCP (`apps/mcp/tools/git.ts`):** both catch the error and surface `err.message` (which names the missing commit + remote and the in-git recovery). P10 parity — added to both, not one.
- **Docs:** CLI + MCP reference each gain a "force-pushed / rewritten branch" subsection.

**Tests (each red-proven — guard disabled → refusal tests fail as take-incoming/GitSyncFirstError; positive fast-forward control stays green; restored).** core `history-rewritten.test.ts` (4: sync refusal names missing commit + nothing written + `last_synced_commit` unchanged + NOT a `GitReconcileNeededError`/`GitConflictError`; unresolvable-base case; publish-path refusal; fast-forward positive control); web `GitSyncPanel.test.tsx` `historyRewritten` detector (3, red-proven); e2e `flow-git-sync.spec.ts` GIT-21 (dedicated banner, names remote+commit, 0 buttons, local task survives, `last_synced_commit` unchanged — red-proven by disabling the guard + rebuild). All tagged `@verifies GIT-21`. A new `forcePushRewriteRemote` fixture helper simulates the force-push.

**Not touched (K93 / task scope):** no-`--force` push guard, `NEVER_MIRROR`/`LOCAL_OWNED`, reconcile/rekey logic; no new `last_synced_commit` write anywhere; GIT-8/35 (K92/K94, separate).

**To revert.** Delete `GitHistoryRewrittenError`, `assertNotHistoryRewrite`, `resolveRemoteHeadForAncestry` and their two call-sites + exports; remove the `gitErrorResponse` branch, the `error()` allowlist line, the contracts `ErrorCode`/`ErrorResponse` additions; remove the panel detector/banner and restore `reconcileBlocked` to the `/reconcil/i` match; drop the CLI/MCP catches; revert the two doc subsections; delete `history-rewritten.test.ts`, the panel detector tests, the GIT-21 e2e + fixture helper. No on-disk format change, so no migration. Reverting re-opens A188 H-e (the live data-loss hazard), so only revert if the detection is being replaced, not removed.

### A192 · GIT-35 newer-remote-schema — read the branch `.schema-version`, refuse if strictly newer, write nothing, per K94

**Ticket:** GIT-35 (K94 is the binding ruling) · **Date:** 2026-09-16 · **Commit:** uncommitted (working tree).

**Situation.** invariants.md holds that schema travels via `loctt migrate`, never via sync. Before A192 that was unenforced on the sync/publish path: a branch published by a *newer* LocTT (its `.schema-version` higher than local `CURRENT_SCHEMA_VERSION`) was applied like any other divergence. `.schema-version` itself is `LOCAL_OWNED`/`NEVER_MIRROR`, so the file was not copied — but every *other* file on the branch (tasks, config, state) written to a newer on-disk shape *was* applied, into a build that cannot read that shape. K94 ruled: before applying a sync, READ the branch's `.schema-version`; if strictly greater than local, throw a distinct error and write nothing. Message says "upgrade LocTT," not "migrate." Only reads the remote value to refuse — never writes it.

**What was built.**
- **Core (`git/publish-sync.ts`):** `GitRemoteSchemaNewerError extends GitSyncError` (carries `remoteVersion`/`localVersion`/`branch`); `assertRemoteSchemaNotNewer(root, remoteHead, branch)` reads `.schema-version` via `readTreeFile` (branch tree, no checkout — the least-invasive of the two sources K94 allowed) and refuses when strictly `> CURRENT_SCHEMA_VERSION`. Wired into **both** paths right beside GIT-21's guard: `pullFromLocttBranch` (sync, before the `last_synced === remoteHead` no-op so a *first* sync of a newer branch is also refused) and `detectPublishReconcile` (publish, after the rewrite guard, before `planSync`). Uses the same `trueRemoteHead` the rewrite guard resolved. Exported from `git/index.ts` + top-level `index.ts`.
- **Malformed-version posture (implementer call, per K94's "decide the safe posture, justify it").** An absent `.schema-version` → **proceed** (older/legacy remote; absent is the ordinary migrate-forward direction, not this guard's job). An empty file → treated as absent (present-but-unwritten; refusing would brick every future sync over a legacy no-op write). A **malformed** value (non-integer / < 1) → **refuse**, surfaced as `remoteVersion: NaN`. Reasoning: a non-numeric version *cannot be proven ≤ local*, and silently syncing unknown-version data is exactly the hazard the guard exists to prevent (K94's intent); refusing with a clear message degrades better than proceeding blind (corruption-handling-guide.md — the check is object-fatal for the sync, not a silent field drop, and it does not crash: it throws a typed, surfaced error every surface catches). A blob that cannot be read at all (git error) reads as absent and proceeds, matching the surrounding broken-git handling.
- **Contracts:** new `ErrorCode` `"schema_remote_newer"` + optional `ErrorResponse.schema_remote_newer` (`remote_version: number | null`, `local_version`, `branch`); `null` carries the malformed/NaN case.
- **Server (`apps/web/src/server/server.ts`):** `gitErrorResponse` maps it to **409** (matching the GIT-21 sibling's envelope shape — client-actionable refusal, not a server fault), `code: "schema_remote_newer"`, `data_state: "not_saved"`, `recovery: { kind: "none" }`, plus a `schema_remote_newer` payload (NaN → null); `error()` allowlist extended to pass it through.
- **Web panel (`GitSyncPanel.tsx`):** `schemaRemoteNewer()` detector + `SchemaRemoteNewerRefusal` banner — names both versions, states nothing was written, says the fix is to **upgrade LocTT** (not migrate), offers no action controls. Wired into both the publish- and sync-error render chains after the history-rewrite branch.
- **CLI (`apps/cli/commands/git.ts`) + MCP (`apps/mcp/tools/git.ts`):** both catch the error on publish AND sync and surface `err.message` (which names both versions + "upgrade LocTT"). P10 parity — added to both, not one.
- **Docs:** CLI + MCP reference each gain a "branch written by a newer LocTT" subsection.

**Tests (each red-proven).** core `schema-remote-newer.test.ts` (5, `@verifies GIT-35`): sync of CURRENT+1 refuses naming both versions + nothing written + `last_synced_commit` unchanged + NOT `GitReconcileNeededError`/`GitConflictError`; publish of CURRENT+1 refuses; malformed refuses (remoteVersion NaN); absent proceeds; equal proceeds. Red-proof: `parsed > CURRENT` → `false && …` turns the two CURRENT+1 refusals red (sync take-incoming, publish `GitSyncFirstError`); the malformed branch red-proven separately by disabling its arm; positive controls stay green; restored. web `GitSyncPanel.test.tsx` `schemaRemoteNewer` detector (4, red-proven by swapping the code check). Case-index regenerated (1047 cases); `cases:check` up to date; `cases:coverage --require GIT-35` ✓.

**Green test edited (asserted the bug).** `publish-sync.test.ts` "never takes .schema-version from the branch" previously put schema `"999"` on the branch and asserted the sync **succeeded** while keeping local `.schema-version` (relying on NEVER_MIRROR). Under K94 a newer-schema branch must be REFUSED, so that green test was encoding the pre-K94 behaviour for the newer case. Rewrote it to keep its still-valid intent — NEVER_MIRROR/`LOCAL_OWNED` means an *applied* sync never overwrites local's `.schema-version` — using a branch schema at the SAME version number (guard silent) but different bytes; the exact-bytes assertion red-proven by removing `.schema-version` from `LOCAL_OWNED`. Flagged in the commit-message-to-be.
