...
## MSL-22 vs V9: one invalid label colour hides every label

**Found 2026-08-25 while verifying M1.2. Not fixed — it needs a
decision, and both candidate answers are already written down.**

Hand-edit one label's `color` to `notahex` and `GET /api/labels`
returns 500 for the *whole file*:

```
detail: labels[1].color must be a hex color (e.g. #1e6fcb or #f00)
```

Every label then renders as "unknown label" on every row, including the
labels that were fine.

**MSL-22 says the label must survive.** Its bullets: an invalid colour
"does not produce an unstyled pill, an unreadable pill, or a thrown
render error", and the bad value is "surfaced in Settings → Labels as a
fixable config problem naming the label and the bad value".

**V9 says config is not a log** — a definition other data references is
not kept and merged like a comment; LocTT refuses rather than building
on top of it.

**Both are defensible and they disagree here.** The tension is narrower
than it looks: V9's reasoning is about a definition that *cannot
render* — a status with no key breaks every task pointing at it. A bad
*colour* does not break the definition. The label still has an id and a
name, and MSL-22's neutral-pill fallback is exactly "render it anyway".

So the question is whether V9 applies per-file or per-field, and that
is a spec decision, not something derivable from the cases. An agent
that picks one has written the spec.

**What is already done:** the client guards the colour, so a bad hex
that *does* reach it takes the neutral default rather than emitting
`notahex22` as CSS (MSL-26's commit). What is missing is the loader
half — whether `loadLabelsConfig` drops the bad field, drops the entry,
or keeps refusing.

**MSL-22 stays untagged** until that is settled.
