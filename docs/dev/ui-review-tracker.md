...
|---|---|---|---|---|
| GAP-SPR-ARCH | server — sprint archive | B2 Sprints lane found sprint **archive/unarchive has no web route** — core `archiveSprint`/`unarchiveSprint` exist but `handleUpdateSprint` doesn't accept `archived` and no endpoint exists (A149/known-gaps). SPR-40 create/delete shipped; archive is the remaining piece. **Follow-up (part of B2 close):** add `POST /api/sprints/:id/archive` + `/unarchive` mirroring labels, `useArchiveSprint` hook + per-row button. Small, server+client. **RESOLVED (B2):** routes + hook + button + tests built (red-first), SPR-40 case updated to the route-based design, A149 resolved. | P2 | ✅ done (B2) |
