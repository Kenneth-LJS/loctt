...
- [x] **NEW-20 (K75)** — **DONE (both halves).** CLI/MCP: resolver names the ghost default in the ask-state error (core, inherited by CLI+MCP); @verifies NEW-20 red-proven (A-NEW20-CLIMCP). GUI: the create modal's no-default message renders "Settings → Projects" as a deep `<Link>` to `/settings/projects` that closes the modal; `@verifies K75` UI test + unit drift-guard, red-proven. Landed with the K76 deep-linking workstream.
**RESOLVED this session:**
- [x] **A80 prefix rule (K88, supersedes K79)** — **DONE, with a follow-up correction (see A-K88).** Prefix stored bare (uppercase letters, ^[A-Z]{1,10}$); the "-" is inserted at key render (T -> T-1). Strict input (dash rejected). Fixed all key-render sites (keys.ts, reconcile.ts, publish-sync.ts, restore.ts, server/cli/mcp previews) + both client validators + defaults; ~49 fixture files swept. @verifies A80/ONB-19. **Follow-up (this session):** the original sweep verified UNIT suites only and left three holes — `createProject` never validated the prefix (dash accepted on `project create`); ~11 integration/e2e files still asserted the dashed stored-form; and `flow-list`/`sync-preflight` over-stripped *rendered* keys (`BACKEND-1`→`BACKEND1`). All fixed and now verified across unit + integration + e2e + Playwright UI. K88.
...
- [x] **A11Y feature-gaps (A11Y-10, 12, 17, 39, 51)** — ALL DONE. §1 is fully closed; every WCAG-AA case is met.
  ...
  - [x] **A11Y-39 — text-only-zoom-to-200% reflow** — DONE (Ken chose "do the restyle now", superseding A95/A-A11Y39-HOLD). Anchored the base to the browser root (`html { font-size: 87.5% }`, body inherits — a `1rem` on html would cancel the 87.5%), moved the `--text-*` scale + all 860 inline `text-[Npx]` sites (104 files) to `text-[…rem]` (rem = px/14, size-preserving). Tailwind's numeric spacing/height utils are already rem so they scale too; the few literal `h-[Npx]` are square icon badges (graphics, not text), left as-is. `@verifies A11Y-39` spec (text grows at 175% root zoom, nothing clips, no h-scroll), red-proven by reverting a cell to px. Browser-smoke-checked list+board at 100% (identical) and 175% (clean reflow). No visual regression — a size-preserving px→rem swap, not a redesign.
...
- [x] **Saved-view `conditions` migration path (BEFORE public release)** —
  **DONE (A228).** The loader now DERIVES `conditions` from the stored `query`
  on load whenever the `conditions` block is absent or malformed (via core's
  total `queryToConditions`), so a pre-A217 `queries.yaml` loads whole and
  self-heals on the next write (the serializer already emits `conditions`).
  A view recoverable from NEITHER a usable `conditions` block NOR a parseable
  `query` degrades per-view to a `BrokenSavedQuery` marker instead of taking
  down the whole file (closes the matching known-gaps entry too). The load
  step happens in a TOLERANT loader schema (`conditions` accepted as unknown,
  repaired per-entry) BEFORE the strict `SavedQuerySchema` is required, so the
  written/validated shape still requires `conditions`. A new `loctt doctor`
  check reports migrated views ("conditions derived from query — will persist
  on next write") and broken views. `QueriesConfig` gained a `migrated`
  load-time diagnostic (mirrors `broken`; never written to disk). All three
  surfaces (CLI `views`/`list --view`, MCP `list_views`) inherit it via the
  shared `loadQueriesConfig`. `@verifies DEG-25` red-proven at the loader
  (derive-on-missing, derive-on-malformed, persist-on-write, per-view
  degrade, whole legacy file) + doctor (migrated + broken). Ties known-gaps
  "conditions block missing/corrupt is object-fatal" (removed).
...
- [x] **The git-sync engine (GIT-8,9,16,19,21,22,23,25,29,30,33,34,35,36)** — **DONE (all sub-items committed). STALE LINE CORRECTED (A188).** The reconcile engine already exists, is wired on all three surfaces, and is tested (A121/commit e99bfec) — the "no engine exists" framing was written against the superseded A68 ruling. What actually remains is a residual set of scoped features, split by risk:
  - [x] **GIT-29/30** — push/fetch error classification (auth vs non-ff vs unreachable; fixed GIT-30's 200-on-unreachable bug). DONE, commit dde0626, A189.
  - [x] **GIT-23** — 500-task sync progress + honest counts. DONE, commit e66777d (A190). + fixed a pre-existing invalidation-drift bug (list/badges didn't refresh after sync).
  - [x] **GIT-16** — delete-vs-edit reconciliation row. DONE (A196). New `deleteVsEdit` plan kind detected in `computeReconcilePlan` from the file-level deletes/copies + base; keep-deletion / keep-task, reported by key; keep-task colliding routes through the K92 rekey gate. Web panel row + CLI/MCP parity + docs. @verifies GIT-16 at core (5 integration) + web unit (3) + Playwright (2), red-proven.
  - [x] **GIT-22** — fstype enable-warning. DONE, commit 958a466. (e2e pending for cluster gate.)
  - [x] **GIT-34/36** — malformed-remote report + missing-worktree named error. DONE, commit de0019b (A194).
  - [x] **GIT-19** — rekey stale-tab: server `expectedId` precondition prevents a wrong-task write (a real P-11 bug) + UI follow-with-note. DONE, commit 9938221 (A195).
  - [x] **GIT-25** — adopt-existing-branch prompt. DONE, commit d15f9c6 (A197). enable refuses-and-reports a pre-existing LocTT branch (shows head), adopts only on explicit --adopt/param/re-POST, sets last_synced_commit to branch head + reports agreement. Foreign-branch refusal untouched. config-set-enabled also refuses silent adopt now.
  - [x] **GIT-21** — force-push / history-rewrite DETECT + REFUSE per **K93** (A191). `GitHistoryRewrittenError` + `assertNotHistoryRewrite` (checks ancestry against the TRUE remote head — the remote-tracking ref, since a non-ff fetch leaves the local `loctt` ref stale on a rewrite) wired before `planSync` on BOTH the sync (`pullFromLocttBranch`) and publish (`detectPublishReconcile`) paths; refuses with nothing written and `last_synced_commit` untouched, no automated recovery. All surfaces (web 409 `history_rewritten` + dedicated refusal banner, CLI, MCP) + CLI/MCP docs. `@verifies GIT-21` red-proven at unit + e2e. Case lifted into flow-git-sync.md.
  - [x] **GIT-35** — newer-remote-schema DETECT + REFUSE per **K94** (A192). `GitRemoteSchemaNewerError` + `assertRemoteSchemaNotNewer` (reads the branch `.schema-version` via `readTreeFile`, no checkout; refuses when strictly `> CURRENT_SCHEMA_VERSION`) wired before `planSync` on BOTH the sync (`pullFromLocttBranch`, before the no-op short-circuit) and publish (`detectPublishReconcile`) paths; nothing written, `.schema-version` only READ (stays LOCAL_OWNED/NEVER_MIRROR). Malformed → refuse (unknown can't be proven ≤ local); absent/equal/older → proceed. All surfaces (web 409 `schema_remote_newer` + dedicated "upgrade LocTT" refusal banner, CLI, MCP) + CLI/MCP docs. `@verifies GIT-35` red-proven (5 core + 4 panel). Edited one green test that asserted the pre-K94 behaviour (see A192). Case already in flow-git-sync.md; case-index regenerated.
  - [x] **GIT-8/9/33** rekey preview+confirm per **K92** (A193) — DONE. Split a PURE `previewRekey(tasks, state): RekeyPlan` (contracts type) out of `rekeyCollisions`, which is now derived from it (anti-drift proven by a round-trip test). Web reconcile panel shows the rekey preview (keeper vs loser, both created_at, both ULIDs, tiebreak, planned new key) as a second phase of the reconcile session and WAITS for confirm (`GitRekeyNeededError` + `rekey_pending` sentinel + `confirmRekey` + `/api/git/reconcile/confirm-rekey`); nothing renumbered until confirm (asserted disk-unchanged pre-confirm). CLI/MCP auto-apply (`sync(..., { rekeyConfirmed: true })`) and report old→new per task via new `SyncOutcome.rekeys`. GIT-33 partial failure reuses `unresolvedKeys`. @verifies GIT-8/9/33 across contracts + core (previewRekey + anti-drift + confirm-gate + partial) + web server (409 rekey_needed → confirm-rekey) + CLI/MCP integration + Playwright e2e (`flow-git-rekey.spec.ts`), each red-proven. Docs: CLI + MCP + common git-sync. Ties to H2 in release-readiness.md.
- [x] **git-sync UI cases** — DONE. Every GIT-* case (GIT-1..GIT-38 + GIT-C*) now has a `@verifies` test; the stale "declined/no-engine" block in known-gaps.md and the "Gaps only"/"Partly fixed" framings in flow-git-sync.md are corrected to reflect the built state (2026-09-17).
- [x] **Config pickers break past 1000 entries** — **DONE (K90, ties NEW-25; A-K90-SEARCH).** ✅ Backend (`0f82bde`): core `filterByName`/`filterProjects` + `?q=` on all five web endpoints + CLI `--filter`/pagination + MCP `q`/pagination + truncation footer + docs. ✅ Labels picker (`cd54429`): server-queried candidates + server-authoritative create-offer gate — the duplicate-write hazard is closed; `@verifies K90` at 1100 labels, red-proven. ✅ OptionPicker opt-in search (uncommitted): milestone/sprint/assignee/reporter + create-modal project pickers query the server as you type; `@verifies K90` at 1100 milestones, red-proven; the 16 static usages unchanged. A43 superseded, known-gaps entry removed.
...
- [x] **CMT-10 — query on comment mentions** — **DONE (A183).** New `comment_mentions` query field (`=`/`!=`/`in`/`not in`, operand a user id or `currentUser()`), injected into the evaluator like `body` via `EvalContext.commentMentions` so the evaluator stays pure; `loadCommentMentions` + `resolveCommentMentionsContext` (in core `list.ts`) do the comment scan, gated on the query referencing the field so ordinary lists pay no comment I/O; validation restricts the operators and does not enum-check the operand (a deleted-user ULID still matches). Wired on CLI/MCP/web through `listTasks` (P10), and the web "Mentions me" built-in now resolves to `comment_mentions = currentUser()`. `@verifies CMT-10`: evaluator, loader/merge/gate, validation, built-in, and spawned-binary e2e — each red-proven. CLI + MCP query-reference docs updated.
- [x] **CMT-20 — comments list scale affordances** — DONE (A-CMT20). Chose a bounded scroll box over the infinite-query refactor (the case allows "scrollable OR progressively loaded", and paginating `useComments` would change its shape for `ActivityPanel` too): the list renders in a `max-h`/`overflow-y-auto` container so a long thread scrolls within it and the composer stays just below; posting scrolls the box to the new comment; a >1200-char comment clamps behind a Show more toggle (`ClampedBody`). The core+API pagination (`listCommentsPage`, `?offset/limit`) was still built + tested (backward-compatible, ready if true progressive loading is ever wanted). `@verifies CMT-20` UI spec (all 4 bullets, red-proven on the clamp); flow-comments (20) + flow-activity (16) pass unchanged.
- [x] **TSK-12 — custom fields have no task-type scope** — DONE (K91, A-K91; commit 071f3d1). Optional `task_types?` allowlist on CustomFieldDef (absent = global); MetaPanel + create modal filter by type; out-of-scope stored values kept read-only. @verifies TSK-12 across contract/jsdom/UI, red-proven.
...
- [x] **SET-9 — estimate field missing from create modal + list** — **DONE (A-SET9-ESTIMATE-UI).** Added a config-shaped Estimate control to the create modal (hidden when estimation disabled; enum picker vs suffixed numeric input) and an opt-in, estimation-gated `estimate` list column, both driven by a new shared `estimationShape` helper that `MetaPanel` also now consumes (so the three surfaces can't drift). Task detail + sprint/milestone aggregates already existed. `@verifies SET-9`: 3 create-modal UI tests + 1 list-column UI test (all red-proven) + 4 `resolveColumns` unit tests (gate red-proven). The config-panel half (SET-35 + the existing SET-9 aggregate-note test) was already built.
- [x] **SET-29 — diagnostics do not stream** — DONE (A182; commit 7992cc4). NDJSON-streamed `/api/doctor` from a `runDoctorStream` generator (runDoctor still batched for CLI/MCP parity); per-check pending→done UI; SET-40 mid-stream-failure keeps completed checks visible. @verifies SET-29/SET-40, red-proven.
- [~] **MSL-35 — milestone progress cannot fail per row** — AS-BUILT is the honest ceiling; not worth the speculative server work. The per-row affordance is fully built + unit-tested (`ProgressReadout` renders `kind:"unavailable"` — a named error + Retry — in place of `0/0`, distinct from MSL-15's "No tasks"), and the whole-list failure path has a `(MSL-35, partial)` UI test. Bullet 3 ("*other* rows keep rendering") wants per-milestone isolation, but progress is computed from ONE shared task scan — there is no real failure mode where one milestone's progress fails while others succeed. A `progress | {error}` partial-success shape would be building for a failure that cannot occur. Left as documented-partial rather than inventing a speculative per-milestone endpoint. (Revisit only if a real per-milestone computation is ever introduced.)
- [x] **MSL-29 — hand-edited workflow.yaml not seen until refresh** — DONE (verify-close). The TEMP note over-specified this: the *case* only requires that **on refresh** progress reflects the new config with no cached figure surviving — not a live file-watcher. `withProgress` calls `loadWorkflowConfig` fresh per request (no cache), so a recategorised status is picked up on the next fetch. Already covered by a nuanced `@verifies MSL-29` UI spec (recategorise → reload → numerator rises), which passes. A live watcher would be a separate enhancement the case does not ask for.
- [x] **REL-16 — attachment image render** — DONE, commit d4aae6f (K95/A199). Resolved the K14↔REL-18 conflict: store raw (REL-18 byte-identity holds), render the full local image inline via `<img>` on a separate `?inline=1` serve path (image/* only, nosniff, symlink-refused; download stays octet-stream per REL-38). `<img>` sandboxes SVG; `onError`→type-icon fallback (K14 pt 4). No thumbnail pipeline / re-encode / cropper (unnecessary for a local tool). REL-18 test confirmed still green. @verifies REL-16 at core+server+client+e2e, red-proven.
- [x] **PRU-17 — "clear the project field" option** — DONE. `deleteProject` now accepts `clearProjectField` (mutually exclusive with `remapTo`; exactly one required when tasks exist). The `remap_project` journal `to` became nullable (`null` = clear), and the replay drops the `project` key (mirroring milestone/sprint clears). Wired through CLI (`--clear-project-field`), MCP (`clear_project_field`), and the web delete dialog (a remap-vs-clear radio choice). Core tests (clear path + both-choices rejection, red-proven) + a web UI spec (clear choice deletes and the task survives project-less, verified via CLI `show`). Docs updated. (A-PRU17-CLEAR)
...
- [x] **ERR-11 / ERR-12 — typed content survives a failed save** — DONE (verify-close), commit 763d53e (A198). The "needs the body editor" blocker was stale — BodyEditor/useBodyAutosave exist and the buffer-survives-failed-save behavior is real (the catch never resets bufferRef→savedRef); server classification (FsAccessError→io_failed/not_saved/retry, cause named) was already done+tested. ERR-12 (disk-full) client test already existed; added the missing ERR-11 (permission-at-save-time) client test, asserted via retry issuing a fresh POST (write-count+1, not last-body, since a clobbered buffer would false-pass a last-body check). Red-proven. No production code changed.
- [x] **ERR-23 — a failure mid multi-step create names the step + what already applied** — RESOLVED as unsatisfiable-as-written (documented in known-gaps.md "ERR-23 has no multi-step create flow to fail in", re-confirmed 2026-09-18). The web create is a SINGLE atomic `POST /api/tasks` via `toCreateRequest` (labels bundled in the payload; a new label is created BEFORE the task and only its id is attached, so a label failure means no task was created). There is no task-written-then-follow-up-write-fails window, so ERR-23's partial state cannot arise — the guarantee it wants is provided MORE strongly by never splitting the create. Not a defect; verifying it as written would require inventing a worse multi-step create. Becomes live only if a future create gains a distinct follow-up write.
...
- [x] **Coverage gate reads case IDs from prose, not only tags** — DONE. The `VERIFIES` regex in `tools/coverage/scan.ts` matched `@verifies` anywhere on a line, so a case ID in prose ("unlike @verifies LST-3") could be miscounted as coverage. Anchored it to the start of the comment (`^\s*(//|/*|/**|*)?\s*@verifies …`), one tag per line. Verified old vs new both find 1958 tags (zero real tags dropped — all existing tags are already comment-leading); red-proven a prose mention no longer counts.
...
- [x] **Removing a worktree leaves its `loctt ui` server running** — DONE. Added `registerServerChild` to `server-harness.ts`: a live-children set torn down (SIGKILL) on the runner's own SIGINT/SIGTERM/exit, so a suite killed mid-run takes its spawned `loctt ui` servers with it (the moment the orphan was created — nobody watching). Both UI fixtures (tracker + git-tracker, 3 spawn sites) register on spawn and `unregister` in their `finally`. Per the known-gaps "Real fix". (A bare `loctt ui` started outside the harness is still the user's to stop.)
