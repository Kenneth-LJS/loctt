...
export async function countTasksByReferences(
  ...
  for (const t of tasks) {
    ...
    // K90: discarded-category tasks ARE counted here — the reference count
    // is a list/query context and shows discarded (unlike burndown/
    // progress, which exclude them). See decisions.md K90.
    if (kind === "label") {
